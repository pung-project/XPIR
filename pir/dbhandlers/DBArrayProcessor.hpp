...
#ifndef DEF_DBARRPROC
#define DEF_DBARRPROC
...
typedef struct ArrayStream {
  char *stream;
  int requested_offset;
} ArrayStream;

class DBArrayProcessor : public DBHandler
{
private:
	boost::mutex mutex;
...
public:
  DBArrayProcessor(uint64_t len, char *bytes, uint64_t nbStreams); // constructor with filesplitting 1 -> nbStreams
	virtual ~DBArrayProcessor();
